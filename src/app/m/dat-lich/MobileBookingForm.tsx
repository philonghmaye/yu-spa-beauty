  ...
  return (
    <>
      ...
      <div className="m-topbar">
        ...
        <span className="m-topbar-title">{t.bookNow}</span>
      </div>
      ...
      <div style={{ paddingBottom: 90 }}>
        ...
        <div className="m-booking-section">
          <div className="m-booking-service-card">
            <div className="info">
              <h3>{tn(booking.service.name)}</h3>
              <p><FiClock style={{ verticalAlign: 'middle' }} /> {booking.service.duration} {t.minutes} | {formatCurrency(booking.service.price)}</p>
            </div>
          ...
          <div className="m-booking-staff">
            ...
            <div>
              ...
              <div className="rating"><span style={{ color: 'var(--gold)' }}>★</span> {booking.staffRating} ({booking.staffReviewCount} {t.reviews})</div>
            </div>
          </div>
        </div>
        ...
        {/* Date & Time */}
        <div className="m-booking-section">
          <div className="m-booking-section-title">{lang === 'en' ? 'Select Date & Time' : 'Chọn ngày & giờ'}</div>
          <input type="date" className="m-date-input" value={selectedDate} onChange={e => setSelectedDate(e.target.value)} min={today} />
          {selectedDate && (
            <div style={{ marginTop: 12 }}>
              {slotsLoading ? (
                <div style={{ textAlign: 'center', padding: 20, color: 'var(--neutral-400)' }}>{t.submitting}</div>
              ) : timeSlots.length > 0 ? (
                ...
                <p style={{ textAlign: 'center', color: 'var(--neutral-400)', padding: 20 }}>{lang === 'en' ? 'No time slots available' : 'Không có khung giờ'}</p>
              )}
            </div>
          )}
        </div>
        ...
        {/* Payment Method */}
        <div className="m-booking-section">
          <div className="m-booking-section-title">{lang === 'en' ? 'Payment Method' : 'Phương thức thanh toán'}</div>
          <div className="m-payment-option active">
            <div className="icon">💰</div>
            <div className="label">{lang === 'en' ? 'Pay at Counter' : 'Thanh toán tại quầy'}</div>
          </div>
        </div>

        {/* Promo Code */}
        <div className="m-booking-section">
          <div className="m-booking-section-title">{lang === 'en' ? 'Promo Code' : 'Mã giảm giá'}</div>
          <div className="m-promo-input">
            <input placeholder={lang === 'en' ? 'Enter promo code' : 'Nhập mã giảm giá'} value={promoCode} onChange={e => { setPromoCode(e.target.value.toUpperCase()); setPromoResult(null); }} />
            <button onClick={handlePromo} disabled={!promoCode.trim() || promoLoading}>
              {promoLoading ? '...' : (lang === 'en' ? 'Apply' : 'Áp dụng')}
            </button>
          </div>
          {promoResult && (
        ...
        {/* Price Summary */}
        <div className="m-booking-section">
          <div className="m-booking-section-title">{lang === 'en' ? 'Payment Details' : 'Chi tiết thanh toán'}</div>
          ...
          {discount > 0 && (
            <div className="m-price-row" style={{ color: 'var(--accent)' }}>
              <span>{lang === 'en' ? 'Discount' : 'Giảm giá'}</span>
              <span>-{formatCurrency(discount)}</span>
            </div>
          )}
          <div className="m-price-row total">
            <span>{lang === 'en' ? 'Total: 1 service' : 'Tổng: 1 dịch vụ'}</span>
      ...
      <div className="m-bottom-bar" style={{ flexDirection: 'column', gap: 0 }}>
        ...
        >
          {submitting ? t.submitting : t.bookNow}
        </button>
      </div>
    </>
  );
